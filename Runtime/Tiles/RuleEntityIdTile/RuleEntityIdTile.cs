...
namespace Unity.Tilemaps
{
    ...
    public class RuleEntityIdTile : EntityIdTileBase
    {
        ...
        public HashSet<Vector3Int> neighborPositions
        {
            get
            {
                if (m_NeighborPositionsDirty)
                {
                    // The neighbor positions are built by OnValidate. They are only rebuilt here if
                    // this is accessed before the Tile has been initialized.
                    if (!m_Data.neighborPositions.IsCreated)
                    ...
                    m_NeighborPositionsDirty = false;
                }
        ...
        /// <summary>
        ///     Returns whether any TilingRule on this RuleEntityIdTile has Animation output.
        /// </summary>
        internal virtual bool hasAnimationRule => m_Data.hasAnimationRule;
        ...
        public struct RuleEntityIdTileDataStruct : IDisposable
        {
            ...
            /// <summary>
            /// Whether any TilingRule has Animation output
            /// </summary>
            [MarshalAs(UnmanagedType.U1)]
            public bool hasAnimationRule;
        ...
        /// <summary>
        /// The refresh extent of a RuleEntityIdTile is the largest absolute neighbor offset on
        /// each axis, derived from the bounds of its (rotation/mirror-expanded) neighbor positions.
        /// </summary>
        public override int3 maxRefreshExtent
        {
            get
            {
                if (!m_Data.neighborPositions.IsCreated)
                ...
                var b = m_Data.bounds;
                return new int3(
                    math.max(math.abs(b.xMin), math.abs(b.xMax)),
                    math.max(math.abs(b.yMin), math.abs(b.yMax)),
                    math.max(math.abs(b.zMin), math.abs(b.zMax)));
            }
        }
                    ...
                    if (RuleMatches(ref ruleMatch, ref tilemapDataStruct, ref dataStruct, ref pos, ref transform))
                    {
                        ...
                        switch (ruleMatchOutput.spriteOutput)
                        {
                            ...
                            case RuleMatchSerializable.OutputSprite.Random:
                                ...
                                var idx = math.clamp((int) math.floor(pv), 0, l - 1);
                                tileData.spriteEntityId = ruleMatchOutput.sprites[idx];
                                if (ruleMatchOutput.randomTransform != RuleMatchSerializable.TransformMatch.Fixed)
                                    ApplyRandomTransform(ruleMatchOutput.randomTransform, ref transform
                                        , ruleMatchOutput.perlinScale, ref pos, dataStruct.rotationAngle);
                                break;
                        }
        ...
        /// <returns>A Perlin Noise value between 0 and 1 based on the given inputs.</returns>
        [BurstCompile]
        public static float GetPerlinValue(ref int3 position, float scale, float offset)
        {
            var posf = new float2(position.x, position.y);
            // cnoise returns a value in [-1, 1], which is remapped to the [0, 1] range used by callers.
            return noise.cnoise((posf + offset) * scale) * 0.5f + 0.5f;
        }

        /// <summary>
        ///     Applies a randomized transform to the given transform based on the random transform rule.
        /// </summary>
        /// <param name="type">Random transform rule.</param>
        /// <param name="transform">The transform to randomize. This is replaced for rotating rules and
        ///     multiplied by the randomized mirror for mirroring rules.</param>
        /// <param name="perlinScale">The Perlin Scale factor of the Tile.</param>
        /// <param name="position">Position of the Tile on the Tilemap.</param>
        /// <param name="rotationAngle">Angle in which the Tile is rotated by in Degrees.</param>
        [BurstCompile]
        public static void ApplyRandomTransform(RuleMatchSerializable.TransformMatch type
            , ref AffineTransform transform
            , float perlinScale
            , ref int3 position
            , int rotationAngle)
        {
            var perlin = GetPerlinValue(ref position, perlinScale, 200000f);
            var rotationCount = rotationAngle > 0 ? 360 / rotationAngle : 1;
            switch (type)
            {
                case RuleMatchSerializable.TransformMatch.MirrorXY:
                    transform = math.mul(transform
                        , math.AffineTransform(int3.zero, quaternion.identity
                            , new float3(math.abs(perlin - 0.5f) > 0.25f ? 1f : -1f, perlin < 0.5f ? 1f : -1f, 1f)));
                    break;
                case RuleMatchSerializable.TransformMatch.MirrorX:
                    transform = math.mul(transform
                        , math.AffineTransform(int3.zero, quaternion.identity
                            , new float3(perlin < 0.5f ? 1f : -1f, 1f, 1f)));
                    break;
                case RuleMatchSerializable.TransformMatch.MirrorY:
                    transform = math.mul(transform
                        , math.AffineTransform(int3.zero, quaternion.identity
                            , new float3(1f, perlin < 0.5f ? 1f : -1f, 1f)));
                    break;
                case RuleMatchSerializable.TransformMatch.Rotated:
                {
                    var angle = math.clamp((int) math.floor(perlin * rotationCount), 0, rotationCount - 1) *
                                rotationAngle;
                    transform = math.AffineTransform(int3.zero, quaternion.Euler(0f, 0f, math.radians(-angle))
                        , new float3(1f));
                    break;
                }
        ...
        {
            var dataStruct = UnsafeUtility.AsRef<RuleEntityIdTileDataStruct>(data);
            if (!dataStruct.hasAnimationRule)
                return;
        ...
        /// <summary>
        ///     The data structure holding the Rule information for matching Rule Entity Id Tiles with
        ///     its neighbors.
        /// </summary>
        [Serializable]
        public class TilingRuleOutput
        {
            ...
            /// <summary>
            ///     The enumeration for matching Neighbors when matching Rule Entity Id Tiles
            /// </summary>
            public class Neighbor
            {
                /// <summary>
                ///     The Rule Entity Id Tile will check if the contents of the cell in that direction is an instance of
                ///     this Rule Entity Id Tile.
                ///     If not, the rule will fail.
                /// </summary>
                public const int This = 1;

                /// <summary>
                ///     The Rule Entity Id Tile will check if the contents of the cell in that direction is not an instance
                ///     of this Rule Entity Id Tile.
        ...
        public class TilingRule : TilingRuleOutput
        {
            ...
            public override void ToRuleMatchOutput(ref RuleMatchOutput output)
            {
                ...
                output.randomTransform = m_RandomTransform;
        ...
        /// <summary>
        ///     Attribute which marks a property which cannot be overridden by an overriding Tile
        /// </summary>
