...
{
    /// <summary>
    ///     The Editor for an AnimatedEntityIdTile.
    /// </summary>
    [CustomEditor(typeof(AnimatedEntityIdTile))]
    [MovedFrom(true, "UnityEditor.Tilemaps.Experimental", "Unity.2D.Tilemap.Editor.Experimental")]
    public class AnimatedEntityIdTileEditor : Editor
    {
        private static readonly string k_UndoName = L10n.Tr("Change AnimatedEntityIdTile");
        ...
        private AnimatedEntityIdTile tile => target as AnimatedEntityIdTile;
        ...
        {
            var count = tile.animatedSprites != null ? tile.animatedSprites.Length + 1 : 1;
            ResizeAnimatedSpriteList(count);

            if (list.index == 0 || list.index < list.count)
            {
                Array.Copy(tile.animatedSprites, list.index + 1, tile.animatedSprites, list.index + 2,
                    list.count - list.index - 1);
                tile.animatedSprites[list.index + 1] = null;
                if (list.IsSelected(list.index))
                    list.index += 1;
            }
            else
            {
                tile.animatedSprites[count - 1] = null;
            }
        }

        private void OnRemoveElement(ReorderableList list)
        {
            if (tile.animatedSprites != null && tile.animatedSprites.Length > 0 &&
                list.index < tile.animatedSprites.Length)
            {
                var sprites = new List<Sprite>(tile.animatedSprites);
        ...
        private void HandleDragAndDrop(Rect guiRect)
        {
            ...
            switch (Event.current.type)
            {
                ...
                case EventType.DragPerform:
                {
                    ...
                    Undo.RegisterCompleteObjectUndo(tile, "Drag and Drop to Animated Entity Id Tile");
                    ResizeAnimatedSpriteList(dragAndDropSprites.Count);
                    Array.Copy(dragAndDropSprites.ToArray(), tile.animatedSprites, dragAndDropSprites.Count);
                    DragAndDropClear();
                    GUI.changed = true;
                    EditorUtility.SetDirty(tile);
                    GUIUtility.ExitGUI();
        ...
        /// <summary>
        ///     Draws an Inspector for the AnimatedEntityIdTile.
        /// </summary>
        public override void OnInspectorGUI()
        {
            ...
            var count = EditorGUILayout.DelayedIntField("Number of Animated Sprites",
                tile.animatedSprites != null ? tile.animatedSprites.Length : 0);
            ...
            {
                var minSpeed = EditorGUILayout.FloatField(Styles.minimumSpeedLabel, tile.minSpeed);
                var maxSpeed = EditorGUILayout.FloatField(Styles.maximumSpeedLabel, tile.maxSpeed);
                if (minSpeed < 0.0f)
                    minSpeed = 0.0f;
                ...
                using (new EditorGUI.DisabledScope(tile.animatedSprites == null
                                                   || (0 < tile.animationStartFrame
                                                       && tile.animationStartFrame <= tile.animatedSprites.Length)))
                {
                    tile.animationStartTime =
                        EditorGUILayout.FloatField(Styles.startTimeLabel, tile.animationStartTime);
                }

                tile.animationStartFrame =
                    EditorGUILayout.IntField(Styles.startFrameLabel, tile.animationStartFrame);
                tile.tileColliderType =
                    (Tile.ColliderType)EditorGUILayout.EnumPopup(Styles.colliderTypeLabel, tile.tileColliderType);
                tile.tileAnimationFlags =
                    (TileAnimationFlags)EditorGUILayout.EnumFlagsField(Styles.flagsLabel, tile.tileAnimationFlags);
            }
        ...
        private static class Styles
        {
            ...
            public static readonly GUIContent emptyAnimatedTileInfo =
                EditorGUIUtility.TrTextContent(
                    ...
                    " to start creating an Animated Entity Id Tile.");
            ...
            public static readonly GUIContent startTimeLabel = EditorGUIUtility.TrTextContent("Start Time",
                "The starting time of this Animated Entity Id Tile. This allows you to start the Animation from a particular time.");

            public static readonly GUIContent startFrameLabel = EditorGUIUtility.TrTextContent("Start Frame",
                "The starting frame of this Animated Entity Id Tile. This allows you to start the Animation from a particular Sprite in the list of Animated Sprites.");
