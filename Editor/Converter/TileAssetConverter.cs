        ...
        public static void Convert(List<TileBase> tileAssetsToConvert, Type tileType)
        {
            ...
            try
            {
                ...
                foreach (var tileAssetToConvert in tileAssetsToConvert)
                {
                    ...
                    if (String.IsNullOrWhiteSpace(assetPath) || String.IsNullOrWhiteSpace(metaFilePath))
                    {
                        Debug.LogWarningFormat("{0} is not a valid asset and cannot be converted", tileAssetToConvert);
                        continue;
                    }
            ...
            if (count > 0)
            {
                ...
                */
                ...
                {
                    var tile = AssetDatabase.LoadAssetAtPath(assetPath, tileType) as TileBase;
                    if (tile == null)
                    {
                        Debug.LogErrorFormat("Failed to convert asset at {0}", assetPath);
                        continue;
                    }
                    if (tile.GetType() != tileType)
                    {
                        Debug.LogErrorFormat("{0} is wrong asset type", tile);
                    }
                    tile.OnEnable();
                }
