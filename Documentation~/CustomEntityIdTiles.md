...
2. Create a class that inherits from `EntityIdTileBase`.
...
4. Override the `structType` property to return the type of the struct. For example:

    ```csharp
    public override Type structType { get => typeof(MyTileData); }
    ```
    
5. Override the `OnEnable` method to initialize the tile data. For example:
...
6. Implement the `CopyDataStruct` method. In the method, use the `CopyStructureToPtr` API to copy data from your tile data instance to a pointer for the Jobs system.

    ```csharp
    public override unsafe void CopyDataStruct(Tilemap tilemap, void* pointerForJobsSystem)
    {
        UnsafeUtility.CopyStructureToPtr(ref tileData, pointerForJobsSystem);
    }
    ```

7. Implement the `RefreshTileJobDelegate` delegate method that the Jobs system uses to refresh tilemap positions when you place a tile. For example:
...
8. Implement the `GetTileDataJobDelegate` delegate method that the Jobs system uses to get tile data. For example:
    ...
    Use the `AsRef` API to read your tile data from the `data` pointer, then set the fields of each `TileData` in the `outTileData` array.

9. Implement the `GetTileAnimationDataJobDelegate` delegate method if your tile has animated properties. Otherwise implement `null`. For an example, refer to the `AnimatedEntityIdTile` script.
...
For more information, refer to [Scriptable tiles](https://docs.unity3d.com/Manual/tilemaps/tiles-for-tilemaps/scriptable-tiles/scriptable-tiles.html) and the [`TileBase`](https://docs.unity3d.com/ScriptReference/Tilemaps.TileBase.html) API. 

## Example

For a full example, in the **Project** window, go to the `Packages/com.unity.2d.tilemap.extras/Runtime/Tiles/AnimatedEntityIdTile` folder, then open the `AnimatedEntityIdTile.cs` script. 
